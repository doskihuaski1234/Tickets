...
export const Dashboard: React.FC<DashboardProps> = ({ tickets }) => {
  ...
  const enProgreso = tickets.filter(t => t.status === 'en_proceso').length;
  ...
  const stats = [
    ...
    { label: 'En Progreso', value: enProgreso, color: 'bg-blue-100 text-blue-700', icon: '⏳' },
    { label: 'Resueltos', value: resueltos, color: 'bg-green-100 text-green-700', icon: '✓' },
  ...
  return (
    <div className="page-container">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold text-slate-800 mb-4">Tickets Recientes</h2>
        {tickets.length === 0 ? (
          <p className="text-slate-500">No hay tickets aún</p>
        ) : (
          <div className="space-y-3">
            {tickets.slice(0, 5).map(ticket => (
              <div key={ticket.id} className="flex items-center justify-between p-3 border border-slate-200 rounded hover:bg-slate-50">
                ...
                <div className="text-right">
                  <span className={`px-3 py-1 rounded text-xs font-semibold ${
                    ...
                    {ticket.status === 'abierto' ? 'Abierto' :
                     ticket.status === 'en_proceso' ? 'En Progreso' :
                     'Resuelto'}
                  </span>
                </div>
              </div>
